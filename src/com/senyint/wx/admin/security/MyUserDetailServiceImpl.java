package com.senyint.wx.admin.security;
...
public class MyUserDetailServiceImpl implements UserDetailsService {
	...
	@Autowired
	private com.senyint.wx.admin.dao.UserDao userDao;
	// 登录验证
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
//		System.err.println("-----------MyUserDetailServiceImpl loadUserByUsername ----------- ");
		
		//取得用户的权限
		com.senyint.wx.admin.entity.User user = userDao.findByUsername(username);
		if  (user==null)  
	...
	// 取得用户的权限
	private Set<GrantedAuthority> obtionGrantedAuthorities(com.senyint.wx.admin.entity.User user) {
		List<Role> roles = user.getRoles();
		Map<Integer,Resource> resources = new HashMap<Integer,Resource>();
		for (Role role : roles) {
			for (Resource resource : role.getResources()) {
				if(!resources.containsKey(resource.getPoid())){
					resources.put(resource.getPoid(), resource);
				}
			}
		}
		...
		for (Resource res : resources.values()) {
			...
			authSet.add(new SimpleGrantedAuthority("ROLE_" + String.valueOf(res.getResKey())));
		}
		return authSet;
	}
}